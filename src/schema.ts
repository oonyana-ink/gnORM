...
export const Schema = (name: string, ...SchemaConfigs: SchemaConfig[]): SchemaInstance => {
    const _fieldsets: SchemaFields[] = SchemaConfigs.map(config => config.fields)
    const getFields = (): SchemaFields => {
        const resolvedFields: SchemaFields = {}
        _fieldsets.forEach((fieldset: SchemaFields) => {
            Object.entries(fieldset).forEach(([key, value]) => {
                resolvedFields[key] = value as TypeDefinition
            })
        })
    ...
    const schemaInstance: SchemaInstance = {
        name,
        get fields () { return getFields() },
        get fieldKeys () { return Object.keys(getFields()) }
    }
    
    registry.registerSchema(schemaInstance) 
